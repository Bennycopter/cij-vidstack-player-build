import { DOMEvent, EventsTarget, fscreen, ViewController, EventsController, onDispose, signal, listenEvent, peek, isNumber, isString, useContext, createContext, Component, functionThrottle, isKeyboardClick, isTouchEvent, setStyle, isFunction, setAttribute, isDOMNode, effect, untrack, functionDebounce, isArray, isKeyboardEvent, deferredPromise, isUndefined, prop, method, provideContext, animationFrameThrottle, uppercaseFirstChar, camelToKebabCase, computed, tick, scoped, noop, State, isNull, ariaBool as ariaBool$1, isWriteSignal, hasProvidedContext, isObject, useState, createScope, r, wasEnterKeyPressed, isPointerEvent, isMouseEvent, kebabToCamelCase, createDisposalBin } from './chunks/vidstack-BUJn1if6.js';
export { appendTriggerEvent, findTriggerEvent, hasTriggerEvent, walkTriggerEventChain } from './chunks/vidstack-BUJn1if6.js';
import { canOrientScreen, isTrackCaptionKind, TextTrackSymbol, TextTrack, isAudioSrc, isVideoSrc, isHLSSupported, isHLSSrc, isDASHSupported, isDASHSrc, IS_CHROME, preconnect, boundTime, canChangeVolume, softResetMediaState, getTimeRangesEnd, updateTimeIntervals, TimeRange, mediaState, getRequestCredentials, watchActiveTextTrack, isCueActive } from './chunks/vidstack-CshLUi9f.js';
export { AUDIO_EXTENSIONS, AUDIO_TYPES, DASH_VIDEO_EXTENSIONS, DASH_VIDEO_TYPES, HLS_VIDEO_EXTENSIONS, HLS_VIDEO_TYPES, VIDEO_EXTENSIONS, VIDEO_TYPES, canGoogleCastSrc, canPlayHLSNatively, canRotateScreen, canUsePictureInPicture, canUseVideoPresentation, findActiveCue, getDownloadFile, getTimeRangesStart, isMediaStream, normalizeTimeIntervals, parseJSONCaptionsFile, watchCueTextChange } from './chunks/vidstack-CshLUi9f.js';
...
class TextRenderers {
  ...
  #customRenderer = null;
  // Region [CIJ] forceRefreshCaptions
  forceRefreshCaptions() {
    this.#renderers.filter((renderer) => renderer instanceof CaptionsTextRenderer).forEach((renderer) => renderer.forceRefresh(this.#textTracks));
  }
  // End [CIJ]
  constructor(media) {
    this.#media = media;
...
class YouTubeProviderLoader {
  ...
  async loadPoster(src, ctx, abort) {
    const { findYouTubePoster, resolveYouTubeVideoId } = await import('./chunks/vidstack-Dm1xEU9Q.js');
  ...
  async requestGoogleCast(trigger) {
    try {
      ...
      if (!this.#googleCastLoader) {
        const $module = await import('./chunks/vidstack-DUl5U76j.js');
